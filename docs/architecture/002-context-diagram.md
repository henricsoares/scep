...
In the current Version 1 runtime, a Researcher or Platform Administrator manages Simulation Runs.
Actors with existing domain permissions provision the required identities and infrastructure. The
external Simulation Engine executes Researcher-defined scenarios by using the participating
EVDriver JWTs and run-scoped contracts.
...
* publish complete recurring weekly occupancy profiles through the authorized SPEC-012 contract.

In the MVP, this environment may be represented by local notebooks and Python scripts.

The Backend API validates, persists and serves accepted prediction publications. It does not train
models or execute inference. Driver-facing consumers receive recurring-pattern results only after
owning-domain eligibility and current operational state are applied.
...
C4Context
    ...
    Rel(aiEnv, scep, "Publishes completed prediction results")
...
The Researcher defines an experiment scenario and may manage its Simulation Run. Actors with the
existing domain permissions provision required identities and infrastructure. The Digital Twin
Simulation Engine executes the scenario and interacts with SCEP through public APIs and run-scoped
contracts. SCEP persists
...
The Data Scientist workflow consumes the actor's own single-Facility `RESEARCH` Dataset Exports and
performs model development in the AI Research Environment. A Data Scientist or explicitly profiled
AI Research Technical Client may act as the authorized prediction publisher and
return a complete 168-bucket Weekly Occupancy Prediction publication to the platform. The first
supported experiment predicts SPEC-010 effective occupancy as a recurring local weekday/hour
pattern, not a specific-date or real-time availability guarantee.
